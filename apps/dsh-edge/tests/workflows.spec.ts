...
describe('repository workflows', () => {
  ...
  it('publishes through the Edge-owned standalone release command', () => {
    ...
    expect(source).toContain('publish:\n    needs: [verify-source, publish-image]\n    permissions:\n      contents: write\n      id-token: write')
  ...
  it('publishes the release Container image once, before npm', () => {
    const release = workflow('release-edge.yml')
    const image = release.slice(release.indexOf('  publish-image:'), release.indexOf('\n  publish:\n'))
    expect(image).toContain('needs: verify-source')
    expect(image).toContain('ref: ${{ needs.verify-source.outputs.release_commit }}')
    ...
    expect(image).toContain('password: ${{ secrets.DOCKERHUB_TOKEN }}')
    expect(image).not.toContain('id-token: write')
    expect(release).toContain('publish:\n    needs: [verify-source, publish-image]')

    const ci = workflow('edge-ci.yml')
    const build = ci.slice(ci.indexOf('  container-image:'), ci.indexOf('\n  verify:\n'))
    expect(build).toContain('load: true')
