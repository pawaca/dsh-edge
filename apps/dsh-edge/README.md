...
The queue accepts up to 128 unsettled inputs and 2 MiB of serialized input data per owner. RPC retries reuse `rpcId`; raw SSE callers can supply an `Idempotency-Key` (up to 128 characters). Reusing an identity with different content fails. Raw SSE starts with a receipt comment and returns `x-dsh-edge-input-id`; disconnecting a stream does not cancel the queued input. Remove a queued input through `session.updateQueue`.
