...
| Other tools | Web Search, filesystem editor tools, MCP, skills, workflows, jobs, and subagents | Search + file + goal + skill tools ported; others not ported | Reuse upstream DeepSeek Web Search with its 30-second tool-call timeout. File tools (read/write/edit/read_image) adapted via `EdgeFileSystem` backed by Computer VFS. Goal tools (create_goal/get_goal/update_goal) via upstream `GoalService` + `ToolGoal` + `GoalRoundDriver`. Skill tool via `SkillRegistry` + `EdgeSkillProvider` (DO KV-backed); owner CRUD at `GET/PUT/DELETE /api/skills`. Add the remaining tools individually against Worker-compatible capabilities; do not advertise unavailable host behavior. |
