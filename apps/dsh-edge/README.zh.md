...
| Other tools | Web Search、filesystem editor tools、MCP、skills、workflows、jobs 和 subagents | Search、文件、goal、skill、MCP 和 workflow 工具已移植 | 复用上游 DeepSeek Web Search 及其 30 秒 tool-call timeout。上游 `workflow` 工具运行在 Edge 自有引擎上：workerd 禁止 `eval`、`new Function`、`node:vm` 和运行时 Wasm 编译，因此脚本在 Direct 与 Dynamic Loader 两种模式下都由 `sval` 在 Durable Object 内解释执行。步数预算会终止失控的解释执行循环（单次原生内建调用内部的工作，例如正则回溯，只受 Workers CPU 与内存上限约束；计划中的升级是可单独终止的 Dynamic Worker 引擎）；同时最多运行 4 个子 agent（低于 Workers 六连接上限）；单次运行最多启动 100 个子 agent，以限制子会话的行写入。文件工具（read/write/edit/read_image）通过 `EdgeFileSystem` 适配 Computer VFS。Goal 工具（`ToolGoal`）作为上游 cordis 插件直接与 `GoalService` 组合。Skill 工具通过 `SkillRegistry` + `EdgeSkillProvider`（DO KV 存储）；owner CRUD 接口 `GET/PUT/DELETE /api/skills`。MCP client（`dsh-mcp-client`）已安装；仅支持 Streamable HTTP transport（Workers 上不可用 stdio）。在 Settings → DSH Edge → MCP Servers 中配置服务器；工具名格式为 `mcp__<serverName>__<tool>`。逐个针对 Worker-compatible capabilities 增加其余工具，不宣称不可用的 host 行为。 |
