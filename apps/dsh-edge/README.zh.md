...
| User questions | `UserQuestionService` answerer waterfall、`dsh-api-remotes` forwarded-event selection 与 Web question composer | 复用并添加 Edge connection seam | 直接安装上游 `UserQuestionService` 和 `dsh-api-remotes` Host entry，让 `ctx.userQuestions.ask()` 把每个 Agent-scoped question 经 gateway 拥有的 `$events` stream 转发到浏览器 composer，答案经 `/api/$events/result` 返回。gateway 会在 socket 断开时保留 pending question，并在重连后重新投递。Edge 只增加一个 `ctx.connection` seam，为 Durable Object 捕获 gateway 的 RPC interceptor，因为 Workers 没有上游 web server。目前尚无工具会提问；plan mode 与 `dsh-tool-ask-user` 是预期的消费者。 |
