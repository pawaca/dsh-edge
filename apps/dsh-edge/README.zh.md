...
| Other tools | Web Search、filesystem editor tools、MCP、skills、workflows、jobs 和 subagents | Search + 文件 + 目标 + 技能工具已移植；其他未移植 | 复用上游 DeepSeek Web Search 及其 30 秒 tool-call timeout。文件工具（read/write/edit/read_image）通过 `EdgeFileSystem` 适配 Computer VFS。目标工具（create_goal/get_goal/update_goal）通过上游 `GoalService` + `ToolGoal` + `GoalRoundDriver`。技能工具通过 `SkillRegistry` + `EdgeSkillProvider`（DO KV 存储）；owner CRUD 接口 `GET/PUT/DELETE /api/skills`。逐个针对 Worker-compatible capabilities 增加其余工具，不宣称不可用的 host 行为。 |
