...
- 2026-08-20: The local alpha.1 release candidate passed the complete repository, standalone, dual-runtime durable-state, snapshot, and installed-tarball gates. Stable and prerelease deployments now remain on `latest` and `next` respectively; the tag workflow requires the tag commit to equal reviewed `master`, publishes npm first, and only then creates the matching GitHub prerelease from reviewed notes. Actual publication remains the final Stage 5 gate.
