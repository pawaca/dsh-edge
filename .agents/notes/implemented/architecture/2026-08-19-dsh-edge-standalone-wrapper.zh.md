...
发布从一个手动请求 workflow 开始；它拥有 repository dispatch authority，但没有 npm 或 OIDC 发布 authority。发布 workflow 从默认分支解析，验证经过 Review 的 `dsh-edge-v*` tag 与 package version 匹配、tag 属于已经 Review 的默认分支历史，重新构建并测试精确 package，在 workspace 外安装 tarball，并在 npm 与 GitHub 写入前分别再次检查 tag 没有移动。npm Trusted Publishing 使用 OIDC provenance，不需要长期 publish token。先发布 npm；只有 notes 与 tarball 相同才会创建匹配的 GitHub Release。
